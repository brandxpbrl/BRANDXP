...
export default function ComingSoon() {
  return (
    <section id="casa-da-vo" className="fragma-case-section fragma-case-section-alt px-6 md:px-10 lg:px-16">
      <div className="mx-auto grid max-w-[1360px] items-start gap-12 xl:grid-cols-[0.72fr_1.28fr] xl:gap-20">
        <div className="max-w-xl">
          <span className="fragma-eyebrow">Case / Property experience</span>
          <h2 className="fragma-display fragma-fluid-heading mt-6">{casaDaVo?.name}</h2>
          <p className="fragma-display fragma-fluid-subtitle mt-6 text-[#514940]">
        ...
        <motion.div
          ...
          className="rounded-[4px] border border-[#D9D1C6] bg-[#FAF7F2] p-4 shadow-[0_24px_70px_rgba(31,31,31,0.1)] sm:p-6"
        >
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {fragmaCasaDaVoAssets.map((asset, index) => {
              const isContained = asset.fit === "contain";

              return (
                <figure key={asset.src} className={index === 0 ? "col-span-2 sm:col-span-2" : "col-span-1"}>
                  <div className="relative aspect-[4/3] overflow-hidden rounded-[2px] bg-[#EEE9E1]">
                    <Image
                      src={asset.src}
                      alt={asset.alt}
                      fill
                      sizes="(max-width: 640px) 50vw, (max-width: 1280px) 30vw, 22vw"
                      className={`${isContained ? "object-contain p-1" : "object-cover"} transition-transform duration-500 hover:scale-[1.02]`}
                    />
                  </div>
                  <figcaption className="pt-2 text-[10px] font-bold uppercase leading-4 tracking-[0.16em] text-[#8E7867]">
                    {asset.label}
                  </figcaption>
                </figure>
              );
