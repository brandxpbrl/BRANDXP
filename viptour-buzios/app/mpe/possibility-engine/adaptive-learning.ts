...
import type { TemporalMemory, PossibilityTrajectory } from "./temporal-memory";

export type TrajectoryPattern = PossibilityTrajectory["historicalPattern"];
...
const hints: Record<AdaptiveSignal, PossibilityOperator[]> = {
  explore_related: ["combine", "scale"],
  diversify: ["mutate", "divide", "cooperate"],
  seek_evidence: ["experiment"],
  deprioritize: ["abandon", "divide"],
  insufficient_history: ["experiment"],
};
...
export function deriveAdaptiveExplorationPolicy(graph: PossibilityGraph, memory: TemporalMemory): AdaptiveExplorationPolicy {
  const guidance = graph.nodes.map((p): AdaptiveGuidance => {
    ...
    const pattern: TrajectoryPattern = trajectory?.historicalPattern ?? "insufficient_history";
    const signal: AdaptiveSignal = pattern === "historically_persistent"
      ? "explore_related"
      : pattern === "historically_variable"
        ? "diversify"
        : pattern === "historically_closed"
          ? "deprioritize"
    ...
    return {
      ...
      pattern,
      signal,
      operatorHints: hints[signal],
      reason: pattern === "historically_persistent"
        ? "La rama mostró continuidad histórica observada; explorar vecindad sin asumir superioridad."
        : pattern === "historically_variable"
          ? "La rama cambió entre ciclos; diversificar transformaciones y buscar condiciones explicativas."
          : pattern === "historically_closed"
            ? "La rama apareció cerrada históricamente; reducir prioridad de búsqueda sin borrar evidencia ni declarar imposibilidad universal."
            : "Historia insuficiente; priorizar observación o experimento antes de adaptar la búsqueda.",
      boundary: "HISTORY_GUIDES_EXPLORATION_NOT_TRUTH",
    };
  });
  const counts = new Map<PossibilityOperator, number>();
  guidance.forEach((g) => g.operatorHints.forEach((o) => counts.set(o, (counts.get(o) ?? 0) + 1)));
