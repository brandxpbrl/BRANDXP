  ...
  return (
    <MpeInterfaceFrame active="SOURCE">
      ...
      <main className="px-4 py-5 sm:px-6">
        ...
        <section className="mt-5 overflow-hidden rounded-2xl border border-white/[0.07] bg-[#07101b]/45"><div className="grid grid-cols-[1.05fr_.7fr_1.45fr_.7fr] gap-3 border-b border-white/[0.07] bg-white/[0.025] px-4 py-3 text-[9px] tracking-[0.14em] text-white/30"><span>ENTITY</span><span>ORGAN</span><span>SOURCE</span><span>STATUS</span></div>{organismNodes.map(node=><div key={node.id} className="grid grid-cols-[1.05fr_.7fr_1.45fr_.7fr] gap-3 border-b border-white/[0.055] px-4 py-4 text-xs last:border-b-0"><span className="font-medium text-white/78">{node.label}</span><span className="text-cyan-200/48">{node.organ}</span><span className="break-words text-white/34">{node.source}</span><span className="text-white/34">{node.status.toUpperCase()}</span></div>)}</section>
